...
<form action="{{route('obra.update',$obra->id)}}" method="POST">
    ...
    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="mb-4">
                            ...
                            <input type="text" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="nombre" name="nombre" value="{{@old('nombre', $obra->nombre)}}">
                        </div>
                        <div class="mb-4">
                            <label for="fechaInicio" class="block text-gray-700 text-sm font-bold mb-2">Fecha Inicio:</label>
                            <input type="date" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="fechaInicio" name="fechaInicio" value="{{@old('fechaInicio', $obra->fechaInicio)}}">
                        </div>
                        <div class="mb-4">
                            <label for="fechaEntrega" class="block text-gray-700 text-sm font-bold mb-2">Fecha Entrega:</label>
                            <input type="date" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="fechaEntrega" name="fechaEntrega" value="{{@old('fechaEntrega', $obra->fechaEntrega)}}">
                        </div>
                        <div class="mb-4">
                            <label for="cantidadMaterial" class="block text-gray-700 text-sm font-bold mb-2">Cantidad Material:</label>
                            <input type="text" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="cantidadMaterial" name="cantidadMaterial" value="{{@old('cantidadMaterial', $obra->cantidadMaterial)}}">
                        </div>
                        <div class="mb-4">
        ...
        <option value="{{$tipo->id}}" {{$tipo->id == $obra->tipo_id ? "selected" : ""}}>{{$tipo->nombre}}</option>
        @endforeach
            </select>
            <div class="mb-4">
        ...
        <option value="{{$usuario->id}}" {{$usuario->id == $obra->usuario_id ? "selected" : ""}}>{{$usuario->nombre}}</option>
        @endforeach
            </select>
            </div>
